...
void LSystem::processPy(unsigned int n,
    ...
    // Convert from std::pair<vec3, vec3> to std::vector<float>
    std::vector<float> convertBranch;
    for (int i = 0; i < preBranches.size(); i++)
    {
        vec3 start = preBranches[i].first; 
        vec3 end = preBranches[i].second;
		convertBranch.push_back(start[0]);
		convertBranch.push_back(start[1]);
		convertBranch.push_back(start[2]);

		convertBranch.push_back(end[0]);
		convertBranch.push_back(end[1]);
		convertBranch.push_back(end[2]);

        branches.push_back(convertBranch);
		convertBranch.clear();
    }

    // Convert from std::pair<vec3, string> to std::vector<float>
    std::vector<float> convertFlower;
    for (int i = 0; i < preFlowers.size(); i++)
    {
        vec3 pos = preFlowers[i].first;
		convertFlower.push_back(pos[0]);
		convertFlower.push_back(pos[1]);
		convertFlower.push_back(pos[2]);

        flowers.push_back(convertFlower);
		convertFlower.clear();
    }
}
